...
掌握SparkCore部分的知识，要掌握Spark的Stage的划分原理，掌握Spark的Shuffle原理，深入理解SparkCore运行流程。

# 二 、课堂主题

本次课主要讲解Spark的调优的内容，这些知识的面试时候的重点，同时也是工作当中的重点，务必要掌握！！
...
这儿我们暂时不给大家演示实际的例子，后面会给大家敲一个实际的案例。
...
![img](http://dbaplus.cn/uploadfile/2018/0822/20180822095236554.jpg)
...
这个情况使用得不多，大家了解即可，有可能我讲了以后，要是没有使用过这个repartitionAndSortWithinPartitions算子的同学，其实没听明白我意思。但是没事，这个不重要。
...
![Spark广播变量原理](assets/Spark广播变量原理.png)

####  1.8 使用Kryo优化序列化性能

spark里面有一种默认，那种性能不好，一是序列化的时候速度慢，二 序列化后体积变化不大。
...
![1570604621691](assets/1570604621691.png)
...
为了能把这个资源调优和jvm调优讲清楚，我们先讲Spark的内存模型。
首先我们得明白一个事，我们平时提交的任务设置的—executor-memory 这是值到底是怎么回事？请看下图。

![1570604255683](assets/1570604255683.png)

在2016年spark1.6版本以前 spark的executor使用的静态内存模型，但是在spark1.6开始，多增加了一个统一内存模型。通过spark.memory.useLegacyMode 这个参数去配置。默认这个值是false，带表用的是新的动态内存模型，如果想用以前的静态内存模型，那么就要把这个值改为true。

##### 2.1  静态内存模型：

![1570604272790](assets/1570604272790.png)

实际上就是把我们的一个executor分成了三部分，一部分是Storage内存区域，一部分是execution区域，还有一部分是其他区域。如果使用的静态内存模型，那么用这几个参数去控制：
...
![img](assets/image2018-11-1_16-39-33.png)



动态内存模型先是预留了300m内存，防止内存溢出。动态内存模型把整体内存分成了两部分，由这个参数表示spark.memory.fraction 这个指的默认值是0.6 代表另外的一部分是0.4然后spark.memory.fraction 这部分又划分成为两个小部分。这两小部分共占整体内存的0.6 .这两部分其实就是：Storage内存和execution内存。由spark.memory.storageFraction 这个参数去调配，因为两个共占0.6。如果spark.memory.storageFraction这个值配的是0.5那说明这0.6里面 storage占了0.5，也就是execution占了0.1 。

统一内存模型有什么特点呢？

Storage内存和execution内存 可以相互借用。不用像静态内存模型那样死板，但是是有规则的：

比如场景一：Execution使用的时候发现内存不够了，然后就会把storage的内存里的数据驱逐到磁盘上。

![1570604662552](assets/1570604662552.png)

比如场景二：一开始execution的内存使用得不多，但是storage使用的内存多，所以storage就借用了execution的内存，但是后来execution也要需要内存了，这个时候就会把storage的内存里的数据写到磁盘上，腾出内存空间。

![1570604675176](assets/1570604675176.png)
...
![1565872653413](assets/1565872653413.png)

![1565872766577](assets/1565872766577.png)
...
每个同学自己用Xmind自己写一下，要做到面试的时候可以脱口而出，而且不仅仅是为了面试，这些知识在工作中用的特别特别的多！！
