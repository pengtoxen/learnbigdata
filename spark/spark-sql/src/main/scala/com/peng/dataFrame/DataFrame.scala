...
//利用反射机制实现把rdd转换成dataFrame
case class Person(id: String, name: String, age: Int)

object CaseClassSchema extends App {
	//1.构建sparkSession对象
	val spark: SparkSession = SparkSession.builder().appName("CaseClassSchema").master("local[*]").getOrCreate()
	//2.获取sparkContext对象
	val sc: SparkContext = spark.sparkContext
	sc.setLogLevel("warn")
	//3.读取文件数据
	val data: RDD[Array[String]] = sc.textFile("").map(_.split(" "))
	//4.定义一个样例类
	//5.将rdd与样例类进行关联
	...
	val count = personDF.filter($"age" > 30).count()
	println("count:"+count)
	//分组
